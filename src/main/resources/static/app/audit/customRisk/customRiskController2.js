angular
    .module('altairApp')
    	.controller("customRiskCtrl",['$rootScope','$scope','mainService','sweet','$state','$stateParams','$ocLazyLoad',
	        function ($rootScope,$scope,mainService,sweet,$state,$stateParams,$ocLazyLoad) {   
    		
	    		$ocLazyLoad.load(['lazy_parsleyjs']).then(function(){
	            ...
	            $scope.addNew=function(i){
	            	/*$ocLazyLoad.load('app/audit/customRisk/customRiskView3.html').then(function(){
						$scope.load_modal = 'app/audit/customRisk/customRiskView3.html';
					});
*/
	                mainService.withdomain('get','/au/withid/MainAuditRegistration/'+$stateParams.issueId)
		   			...
		   			mainService.withdomain('get','/my/core/resource/LutGroupOfFactorOther')
		   				.then(function(data){
		   				    ...
		   				    $scope.rtype_data_options=data;
		   				   
		   			});
	            	modalUriskNew.show();    
	            	
	            	//$scope.selectize_conftype_options = $scope.conftype;
		    	...
		    	mainService.withdomain('get','/core/ujson')
					.then(function(data){
						console.log(data);
						$scope.user_data=data.ujson;		
						$scope.role=$scope.user_data[0].role;
				});
