    	      ...
    	      $scope.submitForm = function($event){
    	    	  if($scope.formdata.id==0){  		  
  	    		  ...
  	    		  mainService.withdata('POST', '/core/create/'+$scope.domain,  $scope.formdata).
	    				then(function(data){	    					
	    					if(data.re==1){
	    						sweet.show('Мэдээлэл', $scope.formdata.reg+'УБ-н дугаартай байгууллага бүртгэгдсэн байна.', 'error');
	    					}
	    					if(data.re==2){
	    						sweet.show('Мэдээлэл', $scope.formdata.reg+'Нэвтрэх нэртэй хэрэглэгч бүртгэгдсэн байна.', 'error');
	    					}
	    					if(data==false){
			        ...
			        function(inputvalue) {			        	
    	    		  ...
    	    		  mainService.withdata('POST', '/core/update/'+$scope.domain,  $scope.formdata).
	    				then(function(data){	    					
	    					...
	    					if(data.re==3){
	    						sweet.show('Мэдээлэл', 'УБ-н дугаар оруулна уу.', 'error');
	    					}
	    					if(data==false){
	    						sweet.show('Анхаар!', 'Алдаа...', 'error');
	    					}
	   				                ...
	   				                dataSource: {
	   				                    ...
	   				                    transport: {
	   				                    	read:  {
	   				                            url: "/core/list/LutDepartment",
	   				                            contentType:"application/json; charset=UTF-8",  
	   				                            data:{"custom":"where parentid="+user_data[0].depid+""},
	   				                            type:"POST"
	   				                        },
	   				                        update: {
	   				                            url: "/core/update/"+$scope.domain+"",
